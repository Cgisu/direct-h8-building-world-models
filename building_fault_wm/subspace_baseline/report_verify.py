"""Standard-library verifier for the reviewer subspace report package."""
...
MANIFEST_SCHEMA = "reviewer-subspace-report-package-v1"
SUMMARY_SCHEMA = "reviewer-subspace-report-summary-v1"
RESULT_SCHEMA = "reviewer-subspace-comparison-result-v1"
...
def verify_report(
    ...
    if (
        ...
        raise ValueError("report manifest identity changed")
    ...
    if (
        ...
        raise ValueError("report inventory digest changed")
    expected_paths = set()
    for item in inventory:
        if not isinstance(item, dict) or set(item) != {"path", "bytes", "sha256"}:
            raise ValueError("report inventory row is invalid")
        path = _safe_path(root, item["path"])
        if item["path"] in expected_paths:
            raise ValueError("report inventory path is duplicated")
        expected_paths.add(item["path"])
        if path.stat().st_size != item["bytes"] or sha256_file(path) != item["sha256"]:
            raise ValueError(f"report artifact changed: {item['path']}")
    observed = {
    ...
    if observed != expected_paths:
        raise ValueError("report file set changed")
    ...
    if (
        ...
        raise ValueError("report result summary changed")
    with (root / "selected_hyperparameters.csv").open(
        newline="", encoding="ascii"
    ) as stream:
        selected = list(csv.DictReader(stream))
    if len(selected) != 3 or {row["selected_state_order"] for row in selected} != {"4"}:
        raise ValueError("report selected-model table changed")
