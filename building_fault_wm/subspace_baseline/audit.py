"""Independent numerical audit for the reviewer subspace comparison."""
...
DEFAULT_AUDIT_ROOT = (
    ...
    / "artifacts/reviewer_subspace_audit_v5"
)
...
def _scores(paired: pd.DataFrame, retain_family: bool = False) -> pd.DataFrame:
    ...
    if len(result) != expected:
        raise ValueError("audit equal-weight score grid is incomplete")
...
def _pair(neural: pd.DataFrame, comparator: pd.DataFrame) -> pd.DataFrame:
    if tuple(comparator.columns) != CORE_COLUMNS or len(comparator) != 207_360:
        raise ValueError("audit comparator core changed")
    ...
    if combined.duplicated([*PAIR_COLUMNS, "arm"]).any():
        raise ValueError("audit comparison has duplicate arm rows")
...
def run_audit(
    ...
    if os.path.lexists(output_root):
        raise FileExistsError(f"refusing to overwrite subspace audit: {output_root}")
    completion = verify_evaluation(evaluation_root)
    neural_path = NEURAL_EVALUATION / v3_run.CORE_NAME
    comparator_path = evaluation_root / "subspace_core.csv"
    ...
    if canonical_sha256(recomputed) != canonical_sha256(recorded):
        raise ValueError("independent subspace result differs from recorded result")
    receipt = {
        "schema": "reviewer-subspace-standalone-audit-v1",
        "complete": True,
        ...
        "subspace_core_file_sha256": sha256_file(comparator_path),
