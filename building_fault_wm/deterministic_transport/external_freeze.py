"""Create and verify the public, revision-pinned v3 GitHub Gist freeze."""
...
DEFAULT_FAILURE_CLOSEOUT = (
    PROJECT_ROOT
    / "artifacts/direct_h8_deterministic_transport_v3_failed_attempt_closeout_v1.json"
)
DEFAULT_READINESS = DEFAULT_FREEZE_ROOT / "collection_readiness.json"
DEFAULT_RECEIPT = DEFAULT_FREEZE_ROOT / "external_freeze_receipt.json"

SCHEMA = "direct-h8-deterministic-transport-v3-github-gist-freeze-v2"
...
FREEZE_FILENAMES = frozenset(
    {
        ...
        "failed_attempt_closeout.json",
        "prelock_preparation_complete.json",
        "prelock_registry.canonical.sha256",
        "prelock_registry.json",
...
def _fetch_json(url: str) -> GitHubResponse:
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "wm-buildings-v3-freeze-verifier",
    ...
    provider_date = parsed_date.astimezone(timezone.utc).isoformat().replace(
        "+00:00", "Z"
    )
    return GitHubResponse(
        payload=_strict_json_bytes(content, "GitHub Gist response"),
        provider_date_utc=provider_date,
    )
...
def _prelock_digest(prelock_root: Path) -> str:
    ...
    if len(record) != 65 or not record.endswith("\n") or not _valid_sha256(
        record[:-1]
    ):
        raise ValueError("v3 prelock digest record is malformed")
    registry = _strict_json_bytes(
        _plain_file(
            prelock_root / "prelock_registry.json", "v3 prelock registry"
        ).read_bytes(),
        "v3 prelock registry",
    )
    if plan.canonical_sha256(registry) != record[:-1]:
        raise ValueError("v3 prelock registry differs from its digest")
    return record[:-1]
...
def freeze_file_paths(
    ...
    bundle = prelock_root / "bundle"
    result = {
        "PROTOCOL.md": bundle / "v3_source/PROTOCOL.md",
        "collection_readiness.json": readiness_path,
        "failed_attempt_closeout.json": DEFAULT_FAILURE_CLOSEOUT,
        "prelock_preparation_complete.json": (
            prelock_root / "prelock_preparation_complete.json"
        ),
        "prelock_registry.canonical.sha256": (
            prelock_root / "prelock_registry.canonical.sha256"
        ),
        "prelock_registry.json": prelock_root / "prelock_registry.json",
        "disjointness_certificate.json": (
            bundle / "plans/disjointness_certificate.json"
        ),
    }
    if set(result) != FREEZE_FILENAMES:
        raise AssertionError("v3 external-freeze file set is incomplete")
    for name, path in result.items():
        _plain_file(path, f"v3 freeze source {name}")
    return result
...
def _readiness_digest(path: Path, expected_prelock_sha256: str) -> str:
    payload = _strict_json_bytes(
        _plain_file(path, "v3 collection readiness").read_bytes(),
        "v3 collection readiness",
    )
    digest = payload.get("readiness_sha256")
    unsigned = {key: value for key, value in payload.items() if key != "readiness_sha256"}
    if (
        not _valid_sha256(digest)
        or plan.canonical_sha256(unsigned) != digest
        ...
        raise ValueError("v3 collection readiness is not a valid pre-outcome lock")
    return digest
...
def _validate_remote(
    ...
    if receipt.get("revision_api_url") != api_url:
        raise ValueError("v3 freeze receipt API URL is noncanonical")
    if receipt.get("revision_html_url") != html_url:
        raise ValueError("v3 freeze receipt HTML URL is noncanonical")
    ...
    if receipt.get("provider_created_at_utc") != created:
        raise ValueError("v3 freeze receipt created_at differs from GitHub")
    if receipt.get("provider_updated_at_utc") != updated:
        raise ValueError("v3 freeze receipt updated_at differs from GitHub")
    ...
    if receipt.get("revision_committed_at_utc") != committed:
        raise ValueError("v3 freeze receipt revision time differs from GitHub")
    ...
    if not isinstance(remote_files, dict) or set(remote_files) != FREEZE_FILENAMES:
        raise ValueError("GitHub Gist file set differs from the v3 freeze contract")
    if set(local_files) != FREEZE_FILENAMES:
        raise ValueError("local v3 freeze file set is incomplete")
    ...
    if receipt.get("file_sha256_by_name") != expected_hashes:
        raise ValueError("v3 freeze receipt file hashes differ")
...
def _validate_receipt_shape(
    ...
    if not isinstance(receipt, dict) or set(receipt) != RECEIPT_FIELDS:
        raise ValueError("v3 external-freeze receipt fields differ")
    if (
        ...
        raise ValueError("v3 external-freeze receipt identity differs")
    _validate_identity(
        receipt.get("gist_id"),
        receipt.get("revision"),
        receipt.get("owner_login"),
    ...
    if (
        ...
        or set(hashes) != FREEZE_FILENAMES
        or any(not _valid_sha256(value) for value in hashes.values())
    ):
        raise ValueError("v3 external-freeze receipt hashes are invalid")
    return receipt
...
def build_external_freeze_receipt(
    ...
    *,
    ...
    readiness_digest = _readiness_digest(readiness_path, prelock_digest)
    ...
    _validate_receipt_shape(receipt, prelock_digest, readiness_digest)
...
def _write_once(path: Path, payload: object) -> None:
    if os.path.lexists(path):
        raise FileExistsError(f"refusing to overwrite v3 freeze evidence: {path}")
...
def validate_external_freeze_receipt(
    ...
    *,
    ...
    receipt = _validate_receipt_shape(
        _strict_json_bytes(
            _plain_file(receipt_path, "v3 external-freeze receipt").read_bytes(),
            "v3 external-freeze receipt",
        ),
        expected_prelock_sha256,
        expected_readiness_sha256,
    )
    if _prelock_digest(prelock_root) != expected_prelock_sha256:
        raise ValueError("live v3 prelock differs from the freeze receipt")
    if (
        _readiness_digest(readiness_path, expected_prelock_sha256)
        != expected_readiness_sha256
    ):
        raise ValueError("live v3 readiness differs from the freeze receipt")
    if live:
        files = read_freeze_files(prelock_root, readiness_path)
        response = _fetch_json(str(receipt["revision_api_url"]))
        _validate_remote(response, receipt, files)
        receipt = {**receipt, "provider_verified_at_utc": response.provider_date_utc}
    return receipt
...
def create_public_gist(
    *,
    ...
    command = [
        ...
        "--public",
        "--desc",
        "Building world-model long-horizon supervision transport study freeze",
    ...
    revision = history[0].get("version")
